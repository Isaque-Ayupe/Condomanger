...
if (changePasswordForm) {
   ...
   async function adicionarReserva(reservaData) {
    try {
        ...
        // Se a requisição der erro, capturamos o motivo
        if (!response.ok) {
            const data = await response.json();
        ...
        // Aqui é onde o JSON da RESPOSTA realmente existe
        const json = await response.json();

        // ⬇️ AQUI você coloca o console.log
        console.log("DEBUG: resposta POST /reservas ->", json);
