...
function mostrarDetalhesLog(log) {
    alert(
        "Ação: " + log.acao_log + "\n" +
        "Usuário: " + log.usuario + "\n" +
        "Data/Hora: " + new Date(log.data_hora).toLocaleString() + "\n\n" +
...
if (maintenanceForm) {
 ...
 // =================================================================
// --- LÓGICA DO DASHBOARD (DEBUG E CORREÇÃO) ---
// =================================================================
function atualizarDashboard() {
   ...
   function renderCalendar(amenityId) {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        // Nota: A lógica original de 'dayDate < hojeReal' estava duplicada e não normalizada.
        // A lógica principal agora é tratada pelo bloco 'if (dataDia < hoje)' e 'continue'.
    ...
    // 2. FILTRO DE PRIVACIDADE: somente MINHAS reservas
    const myBookingsForDay = bookingsForDay.filter(r => {
        // Pega o ID do usuário logado e da reserva
        const meuId = window.userId;
        const idDonoReserva = r.usuario || r.id_usuario || r.usuario_id;

        // Debug no console (pode remover depois se quiser)
        console.log("🔍 CHECK:", {
            "Meu ID": meuId,
            "ID Reserva": idDonoReserva,
            "Igual?": String(idDonoReserva) === String(meuId)
        });

        if (!meuId) return false;
        return String(idDonoReserva) === String(meuId);
    });
    ...
    // Se tiver reservas MINHAS, mostra a lista delas
    if (myBookingsForDay.length > 0) {
        bookingList.innerHTML = '';
        
        // AQUI ESTAVA O ERRO ANTES: Agora usamos 'myBookingsForDay' no loop
        myBookingsForDay.forEach(booking => {
            ...
            cancelBtn.onclick = () => {
                reservaIdToCancel = booking.id_reserva || booking.id; 
        ...
        // Configura a visibilidade dos containers
        if(goToAdicionarBtn) goToAdicionarBtn.style.display = availableSlots > 0 ? 'inline-flex' : 'none';
        ...
        if (availableSlots > 0) {
            ...
            `;
            if(goToAdicionarBtn) goToAdicionarBtn.style.display = 'none';
            if(bookingViewContainer) bookingViewContainer.style.display = 'block';
            if(bookingAddView) bookingAddView.style.display = 'none';
        }
    }
    ...
    if (classifiedsGrid) {
        classifiedsGrid.addEventListener('click', (e) => {
            ...
            if (e.target.closest('.edit-btn')) {
                ...
                populateSellerDropdown(editClassifiedModal.querySelector('#edit-classified-seller'), itemData.usuario);
