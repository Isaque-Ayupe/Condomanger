...
async function carregarReservas() {
    try {
        ...
        // Log para checar se os dados vieram limpos (opcional)
        // console.log('Reservas carregadas limpas:', todasAsReservas); 
   ...
   async function adicionarReserva(reservaData) {
    try {
        ...
        // Se deu erro, ler o JSON com a mensagem personalizada
        if (!response.ok) {
   ...
   // script.js (Substitua a sua função deletarReserva)
// ...
// 🎯 FUNÇÃO CORE: DELETAR RESERVA (CORRIGIDA) 🎯
...
amenityViews.forEach(view => {
    ...
    console.warn("--- DEBUG FINAL: ESTADO GLOBAL ---");
    console.log("Moradores carregados:", todosOsMoradores.length);
    console.log("Reservas no Array Global APÓS LOAD:", todasAsReservas.length); // DEVE SER 6
    console.warn("---------------------------------");
